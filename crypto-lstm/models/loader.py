...
def _add_funding_divergence(df: pd.DataFrame, data_dir: str) -> pd.DataFrame:
    ...
    if btc_funding.empty and eth_funding.empty:
        return df
    ...
    df['btc_funding_rate'] = df.get('btc_funding_rate', 0.0).fillna(0.0)
    df['eth_funding_rate'] = df.get('eth_funding_rate', 0.0).fillna(0.0)
